...
# his script serves as a complete pipeline for training and deploying a machine learning model for loan prediction. Depending on the user's choice, it either trains the model on preprocessed data or deploys a pre-trained model for serving predictions. 
# The pipeline includes data preprocessing, model training, and saving the model for deployment.
import datetime
import warnings
...
val = int(input("Train - 0\nDeploy - 1\nEnter your value: ")) #The script prompts the user to enter either 0 (for training the model) or 1 (for deploying the model) 
#script maps categorical variables to numerical values, drops unnecessary features, and changes the data type of the date column
#Two machine learning models are trained: a Silverkite model (greykite_model) and a Neural Prophet model (prophet_model). These models are trained on the preprocessed data.
if val == 0:
    # Loading the data
    ...
    # Deploying the model
    #If the user selects deployment, the script loads a pre-trained model (Neural Prophet) from a specified path using the Deploy.init function. 
    # This function is expected to deploy the model for serving predictions.
    p_path='../output/prophet_model.pkl'
    Deploy.init(p_path)
